...
"""Is the valve AUC inversion a real effect or the degenerate-feature artefact?
    ...
    nice -n 19 .venv/bin/python research/factory/degenerate_feature_probe.py
...
The valve readout of 2026-07-21 (PREREG_DCASE_VALVE_EXTERNAL.md) reported
...
OUT = ROOT / "research" / "factory" / "logs" / "degenerate_feature_probe.md"
...
def main():
    ...
    lines = [
        ...
        "",
        f"Bank: `{BANK.relative_to(ROOT)}` (frozen by PREREG_DCASE_VALVE_EXTERNAL.md).",
    ...
    lines += [
        ...
        f"research/factory/degenerate_feature_probe.py` ({round(time.time() - t0, 1)}s)",
    ]
    OUT.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
